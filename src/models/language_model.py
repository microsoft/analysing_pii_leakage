...
from src.utils.callbacks import EvaluatePerplexityCallback, PrintSampleCallback
...
class LanguageModel:
    ...
    def load(self, verbose: bool = False) -> 'LanguageModel':
        ...
        if self.model_args.model_ckpt:  # always load the checkpoint if provided.
            if verbose:
                print(f"Loading the provided checkpoint from '{self.model_args.model_ckpt}'.")
            ...
            if verbose:
                print(f"Loading a public, pre-trained model.")
            ...
            if verbose:
                print(f"Loading an uninitialized model.")
    ...
    def perplexity(self, data: Union[list, str], offset=0, max_length=0, apply_exp=True, verbose=True,
        ...
        if return_as_list:
            if apply_exp:
                return torch.exp(torch.stack(nlls, 0))
            return torch.stack(nlls, 0)

        if apply_exp:
            return float(torch.exp(torch.stack(nlls, 0).nansum() / ctr).item())
        return float((torch.stack(nlls, 0).nansum() / ctr).item())
